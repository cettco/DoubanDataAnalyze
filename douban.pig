...
location_group_avg = FOREACH location_group_count GENERATE id,mgroup,MyAvg(total,pinum,pjnum) AS level;
location_group_result = LIMIT location_group_avg 200;
STORE location_group_result INTO 'hbase://location_group' USING org.apache.pig.backend.hadoop.hbase.HBaseStorage('info:group info:level');
...
y_s_group_avg = FOREACH y_s_group_count GENERATE id,mgroup AS mmgroup,MyAvg(total,pinum,pjnum) AS level;
STORE y_s_group_avg INTO 'hbase://y_s_group' USING org.apache.pig.backend.hadoop.hbase.HBaseStorage('info:group info:level');
...
s_group_avg = FOREACH s_group_count GENERATE id,mgroup AS mmgroup,MyAvg(total,pinum,pjnum) AS level;
STORE s_group_avg INTO 'hbase://s_group' USING org.apache.pig.backend.hadoop.hbase.HBaseStorage('info:group info:level');
