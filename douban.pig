...
s_group_count = FOREACH s_group GENERATE group AS mgroup,Count(seasons) AS total,SUM(seasons.pi) AS pinum,SUM(season.pj) AS pjnum;
s_group_avg = FOREACH s_group_count GENERATE mgroup AS mmgroup,MyAvg(total,pinum,pjnum) AS level;
